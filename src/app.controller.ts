...
export class AppController {
  ...
  async renderTemplate(@Body() raw): Promise<string> {
    if (typeof raw === 'string' || Buffer.isBuffer(raw)) {
      ...
      const allowedVariables = ['name', 'age']; // Example allowed variables
      ...
      while ((match = variablePattern.exec(text)) !== null) {
        if (!allowedVariables.includes(match[1])) {
          throw new HttpException('Template contains disallowed variables', HttpStatus.BAD_REQUEST);
        }
      }
  ...
  getSecrets(): Record<string, string> {
    ...
    const secrets = {
      codeclimate: process.env.CODECLIMATE_REPO_TOKEN || 'Not Available',
      facebook: process.env.FACEBOOK_TOKEN || 'Not Available',
      google_b64: process.env.GOOGLE_B64 || 'Not Available',
      google_oauth: process.env.GOOGLE_OAUTH || 'Not Available',
      google_oauth_token: process.env.GOOGLE_OAUTH_TOKEN || 'Not Available',
      heroku: process.env.HEROKU_TOKEN || 'Not Available',
      hockey_app: process.env.HOCKEY_APP_TOKEN || 'Not Available',
      outlook: process.env.OUTLOOK_WEBHOOK || 'Not Available',
      paypal: process.env.PAYPAL_ACCESS_TOKEN || 'Not Available',
      slack: process.env.SLACK_TOKEN || 'Not Available'
    };
    return secrets;
  }
